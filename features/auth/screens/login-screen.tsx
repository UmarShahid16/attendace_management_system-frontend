...
const API_BASE = "https://unrollable-psychodelic-dalia.ngrok-free.dev";
...
export function LoginScreen() {
  ...
  const handleLogin = async () => {
    if (!email.trim() || !password.trim()) {
      toast.warning("Email aur password dono bharo.", {
        description: "Dono fields required hain login ke liye.",
      });
      return;
    }
    ...
    const toastId = toast.loading("Signing in...", {
      description: "Credentials verify ho rahe hain.",
    });
    ...
    try {
      ...
      if (data?.code === "AMS_SUCCESS_00") {
        toast.success(data?.message ?? "Login successful!", {
          id: toastId,
          description: "Dashboard par redirect ho rahe hain...",
        });
      ...
      toast.error(data?.message ?? "Login failed.", {
        id: toastId,
        description: "Email ya password check karo aur dubara try karo.",
      });
    } catch {
      toast.error("Server se connect nahi ho paaya.", {
        id: toastId,
        description: "Internet check karo ya thodi der baad try karo.",
      });
    } finally {
            ...
            <div className="rounded-3xl border border-white/10 bg-white/95 p-8 shadow-2xl shadow-sky-950/30 backdrop-blur-sm dark:bg-slate-900/90 sm:p-10">
              <div className="mb-8 text-center lg:text-left">
                ...
                <p className="mt-1.5 text-sm text-slate-500 dark:text-slate-400">
                  Apne credentials se sign in karo
                </p>
              </div>
